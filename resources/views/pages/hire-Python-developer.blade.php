@extends('layout.main')


@section('main-content')
            <div id="particles-js"></div> 
             <div class="header-banner bg-grad has-mask">
                <div class="ax-inner-banner">
                    <div class="banner-intro">
                        <h3>Hire Python Developer</h3>
                    </div>
                </div>
            </div>
        </header>

        <section>
                <div class="tech-abt-sec">
                    <div class="head-sec">
                        <div class="container">
                            <div class="row">
                                <div class="col-md-12">
                                    <h1 class="h2">Hire Python Developer</h1>
                                    <p style="text-align: justify;">
                                    <section>
                <div class="tech-abt-sec">
                    <div class="head-sec">
                        <div class="container">
                            <div class="row">
                                <div class="col-md-12">
                                    <h1 class="h2">Hire Python Developer</h1>
                                    <p style="text-align: justify;">
                                    OrianaTech Solutions offers skilled Laravel developers ready to elevate your web development projects. Our developers are proficient in leveraging the Laravel framework to build robust, scalable, and secure web applications tailored to your unique business needs. With a focus on quality and efficiency, our team delivers top-notch solutions that drive your business forward. Partner with us to harness the power of Laravel and achieve your development goals.
    
                                    </p>
                                </div>
                           </div>
                        </div>
                    </div>
                </div>
    
                <div class="clearfix"></div>
                
            <div class="tech-feature bg-blue section-padding text-white">
                <div class="container">
                    <div class="row">
                        <div class="col-lg-5 arrow-active" style="margin-left:5%;">
                            <h2 class="section-title text-uppercase">Why Hire a Python developer from OrianaTech Solutions Private Limited</h2>
                        </div>
                        <div class="col-lg-7">
                            <div class="ul-icon white-tick" style=" margin-left: 10%;">
                              
                                <ul style="list-style-type:disc;margin-left: 10%;">
                                    <li>
                                        <h4>Expertise in Python Development</h4>
                                    </li>
                                    <li>
                                        <h4>Versatility and Flexibility </h4>
                                    </li>
                                    <li>
                                        <h4>Robust and Scalable Solutions</h4>
                                    </li>
                                    <li>
                                        <h4>Customized Development</h4>
                                    </li>
                                    <li>
                                        <h4>Comprehensive Services & Quality Assurance</h4>
                                    </li>
                                    <li>
                                        <h4>Clear Communication and Collaboration</h4>
                                    </li>
                                    <li>
                                        <h4>Cost-Effective Solutions </h4>
                                    </li>
                                    <li>
                                        <h4>Dedicated Support and Maintenance</h4>
                                    </li>
                                    
                                   
                                    <li>
                                        <h4>No hidden fees</h4>
                                    </li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

                   
            <div class="tech-feature section-padding">
                <div class="container">
                    <div class="row">
                    <div class="col-lg-6 arrow-active">
                            <h2 class="section-title text-lowercase">
                            Our team of developers is well-versed with the following cutting-edge technologies, which are essential for building high-level Python applications:</h2>
                        </div>

                      
                    <div class="col-lg-6">
                            <div class="ul-icon white-tick">
                                <ul style="list-style-type:disc;margin-left: 10%; text-align: justify">
                                <li>
                                    <p style="text-align: justify">Python Frameworks(e.g.,Django, Flask)</p>
                                </li>
                                <li>
                                    <p style="text-align: justify">Data Science Libraries (e.g.,NumPy, Pandas, SciPy)</p>
                                </li>
                                <li>
                                    <p style="text-align: justify">Machine Learning Frameworks (e.g.,TensorFlow, PyTorch, Scikit-learn)</p>
                                </li>
                                <li>
                                    <p style="text-align: justify">Web Development Tools (e.g., HTML, CSS, JavaScript)</p>
                                </li>
                                   
                                    <li>
                                        <p style="text-align: justify">Database Systems (e.g., PostgreSQL, MySQL, MongoDB)</p>
                                    </li>
                                    <li>
                                        <p style="text-align: justify">Cloud Platforms (e.g., AWS, Azure, Google Cloud Platform)</p>
                                    </li>
                                    <li>
                                        <p style="text-align: justify">Containerization (e.g., Docker, Kubernetes)</p>
                                    </li>
                                    <li>
                                        <p style="text-align: justify">DevOps Tools (e.g., Jenkins, GitLab CI/CD)</p>
                                    </li>
                                    
                                       
                                </ul>
                            </div>
                        </div>
                </div>
            </div>
        </div>
                       
                    </div>
                </div>
            </div>
 </section>
 @endsection